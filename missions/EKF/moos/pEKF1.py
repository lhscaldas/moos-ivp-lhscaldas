...
# Modelo de EKF da apostila do Thiago
import pymoos
import time
import sys
import numpy as np
...
class pEKF(pymoos.comms):
    ...
    def __init__(self, params):
        ...
        self.P = 1e8*np.eye(6)
        self.Q = 1e8*np.diagflat([1,1,10,0,0,0])
        e_IMU_spd = 0.12 ** 2
        e_IMU_pos = 1.2 ** 2
    ...
    def calc_F(self,eta,tau,dt):
        ...
        pkp1 = psi + r*dt
        # if abs(-d33*r*abs(r))>abs(self.aux1):
        #     self.aux1 = -d33*r*abs(r)
        # self.aux2 = (m11-m22)*u*v 
        # self.aux3 = Tau_r
        # self.aux4 = m33*dt
        # self.aux5 = r
        # if pkp1>2*np.pi:
