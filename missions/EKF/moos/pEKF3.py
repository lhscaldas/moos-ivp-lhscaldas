...
class pEKF(pymoos.comms):
    ...
    def __init__(self, params):
        ...
        self.add_message_route_to_active_queue('real_queue', 'REAL_V')
        self.add_message_route_to_active_queue('real_queue', 'REAL_R')
        ...
        self.P = 1e6*np.eye(6)
        self.Q = 1e6*np.diagflat([1,1,1,0,0,0])
        e_IMU_spd = 0.12 ** 2
        e_IMU_pos = 1.2 ** 2
        ...
        e_DVL_spd = 0.002 ** 2
        self.R = np.diagflat([e_GPS_spd,e_DVL_spd,e_IMU_spd,0,0,e_GPS_pos,e_IMU_pos,e_GPS_pos,e_IMU_pos,e_gyro_hdg,e_IMU_hdg])
    ...
    def __on_connect(self):
        ...
        return (self.register('DVL_SPEED', 0) and
                ...
                self.register('DESIRED_RUDDER', 0) and
                self.register('REAL_R', 0) and
                self.register('REAL_V', 0))
    ...
    def calc_F(self,eta,tau,dt):
        ...
        ukp1 = u + (-d11*u*abs(u) + m22*r*v + Tau_u)/m11*dt
        vkp1 = v + (-d22*v*abs(v) - m11*r*u + Tau_v)/m22*dt
        rkp1 = r + (-d33*r*abs(r) - (m11-m22)*u*v)/m33*dt
        xkp1 = x + (u*cos(psi)-v*sin(psi))*dt
        ykp1 = y + (u*sin(psi)+v*cos(psi))*dt
        pkp1 = psi + r*dt
        pkp1 = self.gyro_heading
    ...
    def calc_G(self, eta):
        [u, v, r, x, y, psi] = eta
        return np.array([u,u,u,v,r,x,x,y,y,psi,psi])
    ...
    def sensorZ(self):    
        zk = [self.gps_speed, self.dvl_speed, self.imu_speed, self.real_v, self.real_r, self.gps_x, self.imu_x, self.gps_y, self.imu_y, self.gyro_heading, self.imu_heading]
        return np.array(zk)
    ...
    def debug(self, args=list()):
        ...
        print(" ")
        print("pEKF Debug")
        # print(f"tau_u = {args[1][0]}")
        # print(f"tau_v = {args[1][1]}")
        # print(f"tau_r = {args[1][2]}")
        # print(f"u_bar = {args[0][0]}")
        # print(f"v_bar = {args[0][1]}")
        # print(f"r_bar = {np.rad2deg(args[0][2])}")
        # print(f"psi_bar = {yaw2hdg(args[0][5])}")
...
if __name__ == "__main__":
    file = "moos/alfa.moos" # sys.argv[1] 
