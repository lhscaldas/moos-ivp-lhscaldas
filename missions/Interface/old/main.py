...
def parse_click_event(event: tk.Event, canvas: tk.Canvas, width: float, height: float, m_cells: int, n_cells: int):
    ...
    if (i, j) not in celulas_pressionadas:
        ...
        x2 = j * width / n_cells
        y2 = i * height / m_cells
        rectangle = canvas.create_rectangle(x1, y1, x2, y2, width=1, fill="#fb0")
...
def main(path, m, n):
    window = tk.Tk()
    window.title('NACMM Mission Manager')
...
if __name__ == '__main__':
    ...
    rectangles = []
    try:
        caminho = "sepetiba.TIF" # "caldas.png" # input("Digite o nome do arquivo (com extensao): ")
        linhas = "50" # input("Digite a quantidade de linhas: ")
        colunas = "50" # input("Digite a quantidade de colunas: ")
