...
AOF_DP::AOF_DP(IvPDomain g_domain) : AOF(g_domain)
{
...
bool AOF_DP::setParam(const string& param, double param_val)
{
  if(param == "osy") {
    m_osy = param_val;
    m_osy_set = true;
...
double AOF_DP::evalPoint(const vector<double>& point) const
{
