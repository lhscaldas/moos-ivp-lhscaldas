...
USL_App::USL_App()
{
  ...
  m_display_range_pulse = false;
}
//---------------------------------------------------------
// Procedure: OnNewMail

bool USL_App::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool USL_App::OnConnectToServer()
{
  registerVariables();  
  return(true);
}
...
void USL_App::registerVariables()
{
  AppCastingMOOSApp::RegisterVariables();
  Register("NODE_REPORT", 0);
  Register("NODE_REPORT_LOCAL", 0);
  Register("USL_RANGE_REQUEST", 0);
}
...
bool USL_App::handleNodeReport(const string& node_report_str)
{
...
bool USL_App::handleRangeRequest(const string& request)
{
...
void USL_App::postRangePulse(const string& node, const string& color,
...
void USL_App::postNodeRangeReport(const string& rec_name,
...
{  
  ...
  if((m_report_vars == "short") || (m_report_vars == "both")) {
    ...
    Notify("USL_RANGE_REPORT", full_str);
  }

  if((m_report_vars == "long") || (m_report_vars == "both"))
    Notify("USL_RANGE_REPORT_" + toupper(rec_name), str);
  ...
  if((m_rn_algorithm != "") && m_ground_truth) {
    ...
    if((m_report_vars == "short") || (m_report_vars == "both")) {
      string full_str = "vname=" + rec_name + "," + str;
      Notify("USL_RANGE_REPORT_GT", full_str);
    }
    
    if((m_report_vars == "long") || (m_report_vars == "both"))
      Notify("USL_RANGE_REPORT_GT_" + toupper(rec_name), str);
  }
...
double USL_App::getTrueNodeNodeRange(const string& node_a,
				       const string& node_b)
{
...
double USL_App::getTrueNodeHeading(const string& vname)
{
...
double USL_App::getTrueNodeNodeBearing(const string& node_a,
				       const string& node_b)
{
  if((m_map_node_records.count(node_a) == 0) ||
     (m_map_node_records.count(node_b) == 0))
...
double USL_App::getNoisyNodeNodeRange(double true_range) const
{
...
bool USL_App::setPushDistance(string str)
{
...
bool USL_App::setPullDistance(string str)
{
  string left  = biteStringX(str, '=');
  string right = tolower(str); 
...
bool USL_App::setPingWait(string str)
{
  string left  = biteStringX(str, '=');
...
bool USL_App::setReportVars(string str)
{
...
bool USL_App::setRandomNoiseAlgorithm(string str)
{
...
bool USL_App::setAllowableEchoTypes(string str)
{
...
bool USL_App::allowableEchoType(string vehicle_type)
{
...
bool USL_App::setSensorArc(string str)
{
