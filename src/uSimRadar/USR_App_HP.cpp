...
bool USR_App::OnNewMail(MOOSMSG_LIST &NewMail)
{
...
bool USR_App::OnConnectToServer()
{
  registerVariables();  
  return(true);
}
...
void USR_App::registerVariables()
{
  AppCastingMOOSApp::RegisterVariables();
  Register("NODE_REPORT", 0);
  Register("NODE_REPORT_LOCAL", 0);
  Register("USR_RANGE_REQUEST", 0);
}
...
bool USR_App::handleNodeReport(const string& node_report_str)
{
...
bool USR_App::handleRangeRequest(const string& request)
{
...
void USR_App::postRangePulse(const string& node, const string& color,
...
{  
  ...
  if((m_report_vars == "short") || (m_report_vars == "both")) {
    ...
    Notify("USR_RANGE_REPORT", full_str);
  }

  if((m_report_vars == "long") || (m_report_vars == "both"))
    Notify("USR_RANGE_REPORT_" + toupper(rec_name), str);
  ...
  if((m_rn_algorithm != "") && m_ground_truth) {
    ...
    if((m_report_vars == "short") || (m_report_vars == "both")) {
      string full_str = "vname=" + rec_name + "," + str;
      Notify("USR_RANGE_REPORT_GT", full_str);
    }
    
    if((m_report_vars == "long") || (m_report_vars == "both"))
      Notify("USR_RANGE_REPORT_GT_" + toupper(rec_name), str);
  }
...
double USR_App::getTrueNodeNodeRange(const string& node_a,
				       const string& node_b)
{
...
double USR_App::getTrueNodeHeading(const string& vname)
{
...
double USR_App::getTrueNodeNodeBearing(const string& node_a,
				       const string& node_b)
{
  if((m_map_node_records.count(node_a) == 0) ||
     (m_map_node_records.count(node_b) == 0))
...
double USR_App::getNoisyNodeNodeRange(double true_range) const
{
...
bool USR_App::setPushDistance(string str)
{
...
bool USR_App::setPullDistance(string str)
{
  string left  = biteStringX(str, '=');
  string right = tolower(str); 
...
bool USR_App::setPingWait(string str)
{
  string left  = biteStringX(str, '=');
...
bool USR_App::setReportVars(string str)
{
...
bool USR_App::setRandomNoiseAlgorithm(string str)
{
...
bool USR_App::setAllowableEchoTypes(string str)
{
...
bool USR_App::allowableEchoType(string vehicle_type)
{
...
bool USR_App::setSensorArc(string str)
{
